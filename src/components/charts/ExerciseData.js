    ...
    APIManager.get(
      ...
      APIManager.get(
        ...
        completedWorkoutsR.forEach((cw) => {
          if (
            !years.includes(
              this.convertDateTimeFromISO(cw.date).toDateString().split(" ")[3]
            )
          ) {
            const year = this.convertDateTimeFromISO(cw.date)
              .toDateString()
              .split(" ")[3];
            years.push(year);
          }
  ...
  setCorrespondingExerciseList = () => {
    ...
    this.state.cwExerciseList.forEach((exercise) => {
      if (
        exercise.name
          .toLowerCase()
          .includes(this.state.selectedExercise.name.toLowerCase())
      ) {
  ...
  generateGraph = () => {
    ...
    this.state.cwList.forEach((cw) => {
      for (let date of timeframe) {
        if (
          this.convertDateTimeFromISO(cw.date).toDateString().split(" ")[1] ===
            date.month &&
          this.convertDateTimeFromISO(cw.date).toDateString().split(" ")[3] ===
            date.year
        ) {
          cw.results.forEach((result) => {
            correspondingExerciseList.forEach((exercise) => {
              if (result.exerciseId === exercise.id) {
                if (
                  this.state.selectedExercise.format === "reps-bodyweight" ||
                  this.state.selectedExercise.format === "min" ||
                  this.state.selectedExercise.format === "sec"
                ) {
  ...
  render() {
    return (
      <div className="ed-main-container">
        <Select
          className="react-select-com"
          styles={reactSelectStyle}
          placeholder="Exercise"
          options={this.state.reactExerciseSelect}
          onChange={(opt) => this.setExercise(opt.value)}
        />
        <Select
          className="react-select-com"
          styles={reactSelectStyle}
          ...
          onChange={(opt) => this.setStartMonth(opt.value)}
        />
        <Select
          className="react-select-com"
          styles={reactSelectStyle}
          ...
          onChange={(opt) => this.setStartYear(opt.value)}
        />
        <Select
          className="react-select-com"
          styles={reactSelectStyle}
          placeholder="Number of Months"
          options={this.state.lengthOfTimeMonths}
          onChange={(opt) => this.setLengthOfTime(opt.value)}
        />
        <Select
          className="react-select-com"
          styles={reactSelectStyle}
          placeholder="Graph Data"
          options={this.state.graphTypeSelect}
          onChange={(opt) => this.selectGraphType(opt.value)}
        />
        <button className="select-button" onClick={this.generateGraph}>
          Generate Graph
        </button>
          ...
          <div>
            <Line
              data={{
                ...
                datasets: [
                  {
                    label: this.state.yPrimaryLabel,
                    data: this.state.yAxisPrimary,
                    backgroundColor: "rgba(9, 253, 255, 0)",
                    borderColor: "rgba(9, 253, 255, 1)",
                  },
                ],
              }}
              options={{
                  ...
                  yAxes: [
                    {
                      ticks: {
                        ...
                        max:
                          this.state.yAxisPrimary[
                            this.state.yAxisPrimary.length - 1
                          ] + 10 || 50,
                      },
                    },
                  ],
          ...
          <div>
            <Line
              data={{
                ...
                datasets: [
                  {
                    label: this.state.ySecondaryLabel,
                    data: this.state.yAxisSecondary,
                    backgroundColor: "rgba(9, 253, 255, 0)",
                    borderColor: "rgba(9, 253, 255, 1)",
                  },
                ],
                  ...
                  yAxes: [
                    {
                      ticks: {
                        ...
                        max:
                          this.state.yAxisSecondary[
                            this.state.yAxisSecondary.length - 1
                          ] + 10 || 50,
                      },
                    },
                  ],
