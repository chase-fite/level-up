...
class ExerciseEdit extends Component {
  saveExercise = () => {
    let planString = "";
    if (
      this.props.exercise.format === "min" ||
      this.props.exercise.format === "sec"
    ) {
    ...
    APIManager.update("exercises", newExercise).then(
  ...
  render() {
    return (
      <div className="ec-exercise-container">
        ...
        <input
          className="ec-exercise-name-input"
          type="text"
        ...
        {this.props.exercise.plan.split("--").map((set, indx) => {
          return (
            <div key={indx} className="ee-input-container">
              {`set ${indx + 1}:`}&nbsp;
              <input
                type="text"
                className="ee-input"
                defaultValue={set.split(" ")[2]}
                  ...
                  {this.props.exercise.format.split("-")[1] === "bodyweight" ? (
                    ...
                    <>
                      ...
                      <input
                        type="text"
                        className="ee-input"
                        defaultValue={set.split(" ")[4]}
                        ref={`secondInput-${indx + 1}`}
                      ></input>
                      &nbsp;
                      {`${this.props.exercise.format.split("-")[1]}`}
                    </>
        ...
        <FontAwesomeIcon
          icon={faSave}
          className="fa-lg ee-edit"
          onClick={this.saveExercise}
        />
        <FontAwesomeIcon
          icon={faMinusCircle}
          className="fa-lg ee-minus"
