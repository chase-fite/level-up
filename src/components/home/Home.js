...
class Home extends Component {
  ...
  componentDidMount() {
    if (this.props.isAuthenticated()) {
      ...
      APIManager.get(
        `completedWorkouts?userId=${creds.loggedInUserId}&_expand=workout`
        ...
        temp = temp[0];
        if (temp) {
          APIManager.get(`workouts/${temp.workoutId}?_embed=exercises`).then(
            (workout) => {
              this.setState({
                activeWorkout: temp,
      ...
      <>
        {this.state.activeWorkout.workout.name === "" ? (
          <div className="img-container">
            <img
              src={require("../../images/levelUpLogo2.png")}
              alt="level up logo"
            />
          </div>
        ) : (
