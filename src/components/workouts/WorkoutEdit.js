...
class WorkoutEdit extends Component {
  ...
  saveWorkout = () => {
    ...
    APIManager.update(`workouts`, newWorkout).then((workout) => {
      const promiseArray = [];
      ...
      const filteredAddedExercises = this.state.addedExercises.filter(
        (ex) => ex.workoutId !== this.props.workout.id
      );
  ...
  render() {
    return (
      <>
        ...
        <FontAwesomeIcon
          icon={faMinusCircle}
          className="fa-lg we-minus"
          onClick={this.props.editModeOffWithGet}
        />
        <FontAwesomeIcon
          icon={faSave}
          className="fa-lg we-save"
            ...
            return (
              <div key={indx} className="ec-exercise-container">
                ...
                <FontAwesomeIcon
                  icon={faMinusCircle}
                  className="fa-lg"
                  onClick={() => this.removeExerciseFromAdded(exercise)}
                />
              </div>
            );
          })}
