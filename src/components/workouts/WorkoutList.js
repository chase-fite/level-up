...
class WorkoutList extends Component {
  ...
  componentDidMount() {
    ...
    APIManager.get(
      `workouts?userId=${creds.loggedInUserId}&_embed=exercises&_sort=name`
    ).then((workouts) => {
      const filteredWorkouts = workouts.filter(
        (workout) => workout.name !== "unassigned"
      );
      this.setState({
  ...
  handleSearch = (evt) => {
    ...
    APIManager.get(
      ...
      const filteredWorkouts = workouts.filter((workout) => {
        return (
          workout.name !== "unassigned" &&
          workout.name.toLowerCase().includes(this.state.search.toLowerCase())
        );
      });
      this.setState({
        workouts: filteredWorkouts,
  ...
  deleteWorkout = (workout) => {
    ...
    if (confirm === true) {
      ...
      let targetWorkout = this.state.workouts.filter(
      ...
      Promise.all(promiseArray).then(() => {
        APIManager.delete(`workouts/${workout.id}`).then(() => {
          APIManager.get(
            `workouts?userId=${creds.loggedInUserId}&_embed=exercises&_sort=name`
          ).then((workouts) => {
            const filteredWorkouts = workouts.filter(
  ...
  switchComponent = (mode) => {
    switch (mode) {
      ...
      case "edit":
        let workoutToEdit = this.state.workouts.filter(
          (workout) => workout.id === this.state.editEntityId
        );
  ...
  render() {
    return <>{this.switchComponent(this.state.mode)}</>;
  }
}
